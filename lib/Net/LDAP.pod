...
Please note that IPv6 support is considered experimental in IO::Socket::SSL,
which is used of SSL/TLS support, and therey are a few issues to take care
of. See L<IO::Socket::SSL/IPv6> for details.
...
whole operation can be given in a single argument. C<OP> should be
B<add>, B<delete> or B<replace>. C<VALUE> should be either a string or a
...
Returns the host to which he connection was established.
