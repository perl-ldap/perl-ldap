...
Microsoft Windows systems sometimes have a problem storing the module
with the correct name, it will replace the *.tar.gz with *_tar.tar.
To correct the problem, rename the *_tar.tar to *_tar.gz, then ungzip
the file.  At this point you can execute the tar command to expand 
the tar file.
...
B<WARNING:>  The perl-ldap module is stored on sourceforge as a *.gz file.
Microsoft Windows systems sometimes have a problem storing the module
with the correct name, it will replace the *.tar.gz with *_tar.tar.
To correct the problem, rename the *_tar.tar to *_tar.gz, then ungzip
the file.  At this point you can execute the tar command to expand 
...
Most methods in Net::LDAP return a L<Net::LDAP::Message|Net::LDAP::Message> 
