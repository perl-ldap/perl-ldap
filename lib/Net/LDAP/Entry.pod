...
L<Net::LDAP::LDIF/new> allows, with two being treated special:
...
Instead of only returning the values of the exactly matching attribute name,
get the attribute values of all attributes with the base name given.
...
equally named option in the L<attributes|/attributes> method.
...
by C<get_value> may be references to structures held inside the entry
